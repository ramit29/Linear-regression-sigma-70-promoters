...
class tutorial:
    ...
    def POST(self):
        ...
        model.fit(inputResult,totaloutputResult)
        x2 = np.linspace(5.5,9)
        y2 = model.predict(x2[:,None])
        plt.scatter(result,outputResult,color='#9E9E9E')
        plt.scatter(result2,outputResult2,color='#26a69a')
        plt.plot(x2,y2,'r')
        #plt.legend(['Predicted','Observed'])
        #plt.legend(['Predicted','-10','-35'])
        ...
        prediction2 = model.predict(resultP2)
        pre = str(prediction)
        pre2 = str(prediction2)
        fig1 = plt.gcf()
        #plt.show()
        fig1.savefig('books_readtest.png', dpi=100)
        #source_code = """<span class="UserName"><a href="#">Martin Elias</a></span>"""
        #source_code = """<img src="books_read1.png">"""
        #soup = BeautifulSoup(source_code)
        #print soup.a.string
        data_uri = open('books_readtest.png', 'rb').read().encode('base64').replace('\n', '')
        ...
        start = '<h2>Key : </h2><br>'
        key1 = '<p>Predicted : <i id="red" class="fa fa-long-arrow-right" aria-hidden="true"></i></p><br>'
        key2 = '<p> -35 : <i id = "green" class="fa fa-circle" aria-hidden="true"></i></p><br>'
        key3 = '<p> -10 : <i id = "blue" class="fa fa-circle" aria-hidden="true"></i></p><br>'
        img_tag1 = '<p>Input 10:<span> %s </span></p><br>' % s2
        img_tag1a = '<p>Input 35:<span> %s </span></p><br>' % s1 
        img_tag2 = '<p>X value of Input 10:<span> %s </span></p><br>' % resultP
        img_tag2a = '<p>X value of Input 35:<span> %s </span></p><br>' % resultP2
        img_tag3 = '<p>Predicted Value :<span> %s <span/></p>' % pre
        img_tag3a = '<p>Predicted Value :<span> %s <span/></p>' % pre2
        return d1 + graph + img_tag + d2 + d3 + start + key1 + key2 + key3 + img_tag1 + img_tag1a + img_tag2 + img_tag2a + img_tag3 + img_tag3a + d2 + d2
        #return plt.show()
